...
async def process_gstr1(
    ...
    3. Run full GSTR1Engine pipeline:
       - Adapt input headers
       - Validate rows (structured validation with configured rules)
       - Compute tax amounts
       - Classify transactions
       - Generate GSTR-1 tables with sanity checks
    4. Return structured response with both input-level and table-level validation reports
    
    The validation report includes:
    - Input validation: field-level errors and warnings from configured rules
    - Table generation: sanity checks on generated summary totals
    """
    try:
        from india_compliance.gst_india.engine_core.engine import GSTR1Engine
        from india_compliance.gst_india.engine_core.input_adapter import ExcelInputAdapter
        ...
        print("\n" + "=" * 60)
        print("GSTR-1 SUMMARY (from pipeline):")
        print("=" * 60)
        print(f"  B2B: {len(gstr1_data.get('b2b', []))}")
        print(f"  B2CL: {len(gstr1_data.get('b2cl', []))}")
        print(f"  B2CS: {len(gstr1_data.get('b2cs', []))}")
        print(f"  EXP: {len(gstr1_data.get('exp', []))}")
        ...
        # Consolidate validation reports for frontend
        # Merge input and table validation errors/warnings into flat lists
        all_errors = (
...
# ============ GSTR-1 WORKFLOW STATE MANAGEMENT ============

# In-memory workflow state storage (keyed by workspace_id:gstin:return_period)
# In production, this should be persisted in a database
gstr1_workflow_states: Dict[str, Dict[str, Any]] = {}

def get_state_key(workspace_id: str, gstin: str, return_period: str) -> str:
    """Generate unique key for workflow state"""
    return f"{workspace_id}:{gstin}:{return_period}"
...
def _extract_gstr1_tables(source: Any) -> Dict[str, Any]:
    """Extract a GSTR-1 table payload from a state object or API payload."""
    if not isinstance(source, dict):
        return {}

    for candidate_key in ("gstr1_tables", "gstr1_data"):
        candidate = source.get(candidate_key)
        if isinstance(candidate, dict):
            return candidate

    upload_result = source.get("upload_result")
    if isinstance(upload_result, dict):
        candidate = upload_result.get("data")
        if isinstance(candidate, dict):
            return candidate

    candidate = source.get("data")
    if isinstance(candidate, dict) and any(key in candidate for key in GSTR1_SECTION_KEYS):
        return candidate

    if any(key in source for key in GSTR1_SECTION_KEYS):
        return source

    return {}


def _build_upload_result_from_tables(gstr1_tables: Dict[str, Any]) -> Dict[str, Any]:
    """Create a GSTR-1 process-style response from saved tables."""
    summary = gstr1_tables.get("summary", {}) if isinstance(gstr1_tables, dict) else {}
    if not isinstance(summary, dict):
        summary = {}

    total_records = summary.get("total_invoices")
    if total_records is None:
        total_records = sum(
            len(gstr1_tables.get(key, []))
            for key in ("b2b", "b2cl", "b2cs", "exp", "cdnr", "cdnur")
        )
...
def _build_pan_summary_rows(gstr1_tables: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Build a compact PAN summary table from saved GSTR-1 tables."""
    from india_compliance.gst_india.gstr3b_data import sum_table_values
    ...
    for section_key, section_label in section_definitions:
        entries = gstr1_tables.get(section_key, [])
        if not isinstance(entries, list):
            entries = []

        totals = sum_table_values(entries)
        rows.append(
            {
                "Section": section_label,
                "Document Count": summary.get(f"{section_key}_count", len(entries)),
                "Taxable Value": totals.get("taxable_value", 0),
                "IGST": totals.get("igst", 0),
                "CGST": totals.get("cgst", 0),
                "SGST": totals.get("sgst", 0),
                "CESS": totals.get("cess", 0),
                "Total Tax": totals.get("total_tax", 0),
            }
        )
...
def _ensure_mock_gsp_provider() -> None:
    """Make sure the mock GSP provider is available as a safe fallback."""
    try:
        from india_compliance.gst_india.gsp_integration import (
            GSPConfig,
            GSPProvider,
            configure_gsp,
            get_gsp_registry,
        )
        from india_compliance.gst_india.gsp_integration.mock_gsp import MockGSP  # noqa: F401

        registry = get_gsp_registry()
        if registry.get_default_provider() is None:
            configure_gsp(
                GSPConfig(
                    ...
                    username="mock-user",
                    encrypted_password="mock-password",
                    ip_address="127.0.0.1",
                    is_default=True,
                )
            )
    except Exception as exc:
        logger.warning(f"Unable to initialize mock GSP provider: {exc}")


def _get_filing_provider_for_gstin(gstin: str, current_user: Optional[Dict[str, Any]], ip_address: str) -> Any:
    """Get the active filing provider, creating a mock session when needed."""
    from india_compliance.gst_india.gsp_integration import GSPProvider, get_auth_handler

    auth_handler = get_auth_handler()
    session = auth_handler.get_session(gstin)
    if session:
        return session.provider

    _ensure_mock_gsp_provider()

    username = (current_user or {}).get("sub") or gstin.lower()
...
async def get_gstr1_state(
    ...
    - gstin: GSTIN number
    - return_period: Return period (e.g., 042026)
    ...
    try:
        ...
        if state_key in gstr1_workflow_states:
            ...
            logger.info(f"Retrieved GSTR-1 workflow state: {state_key}")
            return {
                "success": True,
                "data": state
            }
        else:
            logger.info(f"No workflow state found for: {state_key}")
            return {
                "success": False,
                "message": "No saved state found for this GSTIN/period"
            }
    except Exception as e:
        logger.exception(f"Error retrieving GSTR-1 state: {str(e)}")
        return {
            "success": False,
            "error": str(e)
        }
...
async def save_gstr1_state(
    ...
    Stores complete workflow state for resuming later.
    ...
    try:
        ...
        upload_result = request_body.get("upload_result")
        if not isinstance(upload_result, dict) and isinstance(gstr1_tables, dict):
            upload_result = _build_upload_result_from_tables(gstr1_tables)
        ...
        state = {
            ...
            "gstin": gstin,
            "return_period": return_period,
            "current_step": request_body.get("current_step"),
            "step_data": request_body.get("step_data", {}),
            "validation_status": request_body.get("validation_status", {}),
            "gstr1_tables": gstr1_tables,
            "upload_result": upload_result,
            "classification_result": request_body.get("classification_result"),
            "validation_result": request_body.get("validation_result"),
            "filing_result": request_body.get("filing_result"),
            "last_saved": request_body.get("last_saved") or datetime.now().isoformat(),
        ...
        gstr1_workflow_states[state_key] = state
        logger.info(f"Saved GSTR-1 workflow state: {state_key}, step={state.get('current_step')}")
        ...
        logger.exception(f"Error saving GSTR-1 state: {str(e)}")
        return {
            "success": False,
            "error": str(e)
        }
...
@app.delete("/api/gstr1/state")
async def delete_gstr1_state(
    workspace_id: str,
    gstin: str,
    return_period: str,
    current_user: Optional[Dict[str, Any]] = Depends(get_current_user_optional)
    ...
    """
    Delete saved GSTR-1 workflow state
    """
    try:
        state_key = get_state_key(workspace_id, gstin, return_period)
        
        if state_key in gstr1_workflow_states:
            del gstr1_workflow_states[state_key]
            logger.info(f"Deleted GSTR-1 workflow state: {state_key}")
            return {
                "success": True,
            ...
            return {
                "success": False,
                "message": "No state found to delete"
            }
    except Exception as e:
        logger.exception(f"Error deleting GSTR-1 state: {str(e)}")
        return {
            "success": False,
            "error": str(e)
        }
...
@app.get("/api/gstr1/state/list")
async def list_gstr1_states(
    workspace_id: str,
    ...
    """
    List saved GSTR-1 workflow states for a workspace/GSTIN
    """
    try:
        # Filter by workspace and optionally by GSTIN
        states = []
        for key, state in gstr1_workflow_states.items():
            if state.get("workspace_id") == workspace_id:
                if gstin is None or state.get("gstin") == gstin:
                    states.append(state)
        
        # Sort by last_saved (newest first) and limit
        states = sorted(states, key=lambda s: (s.get("last_saved") is None, s.get("last_saved", "")), reverse=True)[:limit]
        
        logger.info(f"Listed {len(states)} GSTR-1 workflow states for workspace: {workspace_id}")
        ...
        logger.exception(f"Error listing GSTR-1 states: {str(e)}")
        return {
            "success": False,
            "error": str(e),
            "data": [],
...
@app.put("/api/gstr1/state/tables")
async def quick_save_gstr1_tables(
    request_body: Dict[str, Any],
    current_user: Optional[Dict[str, Any]] = Depends(get_current_user_optional)
):
    """
    Quick save GSTR-1 tables only (incremental save during workflow)
    
    Updates only the gstr1_tables field without affecting other state fields.
    ...
    try:
        ...
        state_key = get_state_key(workspace_id, gstin, return_period)
        existing_state = gstr1_workflow_states.get(state_key, {})
        upload_result = request_body.get("upload_result") or existing_state.get("upload_result")

        if not isinstance(upload_result, dict) and isinstance(gstr1_tables, dict):
            upload_result = _build_upload_result_from_tables(gstr1_tables)
        elif isinstance(upload_result, dict) and isinstance(gstr1_tables, dict):
            upload_result = {**upload_result, "data": gstr1_tables}
        
        if state_key in gstr1_workflow_states:
            # Update only the tables
            gstr1_workflow_states[state_key]["gstr1_tables"] = gstr1_tables
            if isinstance(upload_result, dict):
                gstr1_workflow_states[state_key]["upload_result"] = upload_result
            gstr1_workflow_states[state_key]["updated_at"] = datetime.now().isoformat()
            gstr1_workflow_states[state_key]["last_saved"] = request_body.get("last_saved") or datetime.now().isoformat()
        else:
            # Create new state with just tables
            gstr1_workflow_states[state_key] = {
                "id": state_key,
                "workspace_id": workspace_id,
                "gstin": gstin,
                "return_period": return_period,
        ...
        for key, value in existing_state.items():
            if key not in gstr1_workflow_states[state_key]:
                gstr1_workflow_states[state_key][key] = value
        
        logger.info(f"Quick saved GSTR-1 tables for: {state_key}")
        
        return {
            "success": True,
            "message": "Tables saved",
            "data": gstr1_workflow_states[state_key]
        }
    except Exception as e:
        logger.exception(f"Error quick saving GSTR-1 tables: {str(e)}")
        return {
            "success": False,
            "error": str(e)
        }
...
async def export_gstr1(data: dict):
    ...
    try:
        ...
        gstr1_tables = _extract_gstr1_tables(data) or data.get("gstr1_tables", {})
        return_period = data.get("return_period", "")
        taxpayer_gstin = data.get("taxpayer_gstin", "")
        taxpayer_name = data.get("taxpayer_name", "")
        company_gstin = data.get("company_gstin", "")
        include_hsn = data.get("include_hsn", True)
...
@app.get("/api/gstr1/pan-summary")
async def download_gstr1_pan_summary(
    workspace_id: str,
    gstin: str,
    return_period: str,
    current_user: Optional[Dict[str, Any]] = Depends(get_current_user_optional),
) -> StreamingResponse:
    """Download a workbook summarizing the saved GSTR-1 sections by PAN."""
    state_key = get_state_key(workspace_id, gstin, return_period)
    state = gstr1_workflow_states.get(state_key)

    if not state:
        raise HTTPException(status_code=404, detail="No saved GSTR-1 data found for PAN summary")

    gstr1_tables = _extract_gstr1_tables(state)
    if not gstr1_tables:
        raise HTTPException(status_code=404, detail="No GSTR-1 tables available for PAN summary")

    summary_rows = _build_pan_summary_rows(gstr1_tables)
    output = io.BytesIO()

    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        pd.DataFrame(summary_rows).to_excel(writer, sheet_name="PAN Summary", index=False)

        workbook = writer.book
        worksheet = writer.sheets["PAN Summary"]
        header_format = workbook.add_format({"bold": True, "bg_color": "#4472C4", "font_color": "white"})
        integer_format = workbook.add_format({"num_format": "#,##0"})
        currency_format = workbook.add_format({"num_format": "#,##0.00"})

        for column_index, column_name in enumerate(["Section", "Document Count", "Taxable Value", "IGST", "CGST", "SGST", "CESS", "Total Tax"]):
            ...
            elif column_name == "Document Count":
                worksheet.set_column(column_index, column_index, 16, integer_format)
            else:
                worksheet.set_column(column_index, column_index, 16, currency_format)

    excel_bytes = output.getvalue()
    filename = f"GSTR1_PAN_Summary_{gstin}_{return_period}.xlsx"
...
@app.post("/api/gstr1/sync-from-gstn")
async def sync_gstr1_from_gstn(
    request_body: Dict[str, Any],
    request: Request,
    current_user: Optional[Dict[str, Any]] = Depends(get_current_user_optional),
) -> Dict[str, Any]:
    """Sync a draft GSTR-1 snapshot from the configured GSP provider or saved state."""
    workspace_id = str(request_body.get("workspace_id") or "").strip()
    gstin = str(request_body.get("gstin") or "").strip()
    return_period = str(request_body.get("return_period") or "").strip()

    if not all([workspace_id, gstin, return_period]):
        raise HTTPException(status_code=400, detail="workspace_id, gstin and return_period are required")

    from india_compliance.gst_india.gsp_integration import get_download_service

    ip_address = request_body.get("ip_address") or (request.client.host if request.client else "127.0.0.1")
    provider = _get_filing_provider_for_gstin(gstin, current_user, ip_address)

    download_service = get_download_service()
    try:
        download_result = download_service.download_gstr1(
            gstin=gstin,
            return_period=return_period,
            provider=provider,
        )
    except Exception as exc:
        logger.warning(f"GSTR-1 sync from GSTN failed, using saved state fallback: {exc}")
        download_result = {"success": False, "message": str(exc), "data": {}}

    state_key = get_state_key(workspace_id, gstin, return_period)
    existing_state = gstr1_workflow_states.get(state_key, {})
    synced_tables = _extract_gstr1_tables(download_result) or _extract_gstr1_tables(existing_state)
    ...
    if synced_tables:
        ...
        state.update({
            ...
            "gstin": gstin,
            "return_period": return_period,
            "gstr1_tables": synced_tables,
            "upload_result": _build_upload_result_from_tables(synced_tables),
            "summary_source": "gstn" if download_result.get("success") else existing_state.get("summary_source", "uploaded"),
            "updated_at": datetime.now().isoformat(),
    ...
    records_synced = download_result.get("record_count")
    if records_synced is None:
        records_synced = sum(
            len(synced_tables.get(section, []))
            for section in ("b2b", "b2cl", "b2cs", "exp", "cdnr", "cdnur")
        ) if synced_tables else 0

    return {
        "success": True,
        "message": download_result.get("message", "GSTR-1 sync completed"),
        "sync_id": download_result.get("download_id") or f"SYNC{uuid.uuid4().hex[:12].upper()}",
        "status": download_result.get("status") or ("completed" if synced_tables else "empty"),
        "records_synced": records_synced,
        "data": synced_tables,
    }


@app.post("/api/gstr1/nil-return")
async def select_gstr1_nil_return(
    request_body: Dict[str, Any],
    current_user: Optional[Dict[str, Any]] = Depends(get_current_user_optional),
) -> Dict[str, Any]:
    """Mark a return as NIL in the saved workflow state."""
    workspace_id = str(request_body.get("workspace_id") or "").strip()
    gstin = str(request_body.get("gstin") or "").strip()
    return_period = str(request_body.get("return_period") or "").strip()
    is_nil_return = bool(request_body.get("is_nil_return", True))

    if not all([workspace_id, gstin, return_period]):
        raise HTTPException(status_code=400, detail="workspace_id, gstin and return_period are required")

    state_key = get_state_key(workspace_id, gstin, return_period)
    state = dict(gstr1_workflow_states.get(state_key, {}))
    state.update({
        "id": state_key,
        "workspace_id": workspace_id,
        "gstin": gstin,
    ...
    return {
        "success": True,
        "message": "NIL return selection saved",
        "is_nil_return": is_nil_return,
    }


@app.post("/api/gstr1/summary-source")
async def select_gstr1_summary_source(
    request_body: Dict[str, Any],
    current_user: Optional[Dict[str, Any]] = Depends(get_current_user_optional),
) -> Dict[str, Any]:
    """Persist the summary source selection for the saved workflow state."""
    workspace_id = str(request_body.get("workspace_id") or "").strip()
    gstin = str(request_body.get("gstin") or "").strip()
    return_period = str(request_body.get("return_period") or "").strip()
    source = str(request_body.get("source") or "").strip()

    if source not in {"gstn", "uploaded", "manual"}:
        raise HTTPException(status_code=400, detail="source must be one of gstn, uploaded, manual")
    ...
    return {
        "success": True,
        "message": "Summary source saved",
        "source": source,
    }


@app.delete("/api/gstr1/clear")
async def clear_gstr1_data(
    workspace_id: str,
    gstin: str,
    return_period: str,
    current_user: Optional[Dict[str, Any]] = Depends(get_current_user_optional),
) -> Dict[str, Any]:
    """Clear saved GSTR-1 data for the selected workspace and period."""
    state_key = get_state_key(workspace_id, gstin, return_period)
    if state_key in gstr1_workflow_states:
        del gstr1_workflow_states[state_key]
        return {"success": True, "message": "GSTR-1 data cleared"}

    return {"success": True, "message": "No saved GSTR-1 data found to clear"}


@app.post("/api/gstr1/file")
async def file_gstr1_return(
    request_body: Dict[str, Any],
    request: Request,
    current_user: Optional[Dict[str, Any]] = Depends(get_current_user_optional),
) -> Dict[str, Any]:
    """File GSTR-1 using the saved draft state and the configured GSP provider."""
    workspace_id = str(request_body.get("workspace_id") or "").strip()
    gstin = str(request_body.get("gstin") or "").strip()
    return_period = str(request_body.get("return_period") or "").strip()
    ...
    gstr1_tables = _extract_gstr1_tables(request_body)
    if not gstr1_tables:
        state = gstr1_workflow_states.get(get_state_key(workspace_id, gstin, return_period), {})
        gstr1_tables = _extract_gstr1_tables(state)

    if not gstr1_tables:
        raise HTTPException(status_code=400, detail="No GSTR-1 tables available for filing")

    from india_compliance.gst_india.gstr1_data import to_gstn_json
    from india_compliance.gst_india.gsp_integration import get_filing_service
    ...
    session = None
    try:
        from india_compliance.gst_india.gsp_integration import get_auth_handler

        session = get_auth_handler().get_session(gstin)
    except Exception:
        session = None

    username = (session.username if session else None) or (current_user or {}).get("sub") or gstin.lower()
    gstn_json = to_gstn_json(
        gstr1_tables=gstr1_tables,
        gstin=gstin,
        return_period=return_period,
        username=username,
        fp=return_period,
        gt=float(request_body.get("gt", 0.0) or 0.0),
        cur_gt=float(request_body.get("cur_gt", 0.0) or 0.0),
    )

    filing_service = get_filing_service()
    filing_response = filing_service.file_gstr1(
        gstin=gstin,
        return_period=return_period,
        json_data=gstn_json,
        provider=provider,
    ...
    updated_state.update(
        {
            ...
            "return_period": return_period,
            "gstr1_tables": gstr1_tables,
            "upload_result": existing_state.get("upload_result") or _build_upload_result_from_tables(gstr1_tables),
            "filing_result": filing_response,
            "status": "filed",
            "updated_at": datetime.now().isoformat(),
            "last_saved": datetime.now().isoformat(),
            "created_at": existing_state.get("created_at", datetime.now().isoformat()),
    ...
    gstr1_workflow_states[state_key] = updated_state
...
async def download_gstr1_json(
    ...
    """Generate and stream a GSTR-1 JSON file from cleaned invoice rows."""
    from india_compliance.gst_india.gstr1_data import generate_gstr1_tables, to_gstn_json
    ...
    username = (current_user or {}).get("sub") or taxpayer_name or taxpayer_gstin or "api_user"
    gstn_json = to_gstn_json(
        gstr1_tables=gstr1_tables,
        gstin=taxpayer_gstin or company_gstin,
        return_period=return_period,
        username=username,
        fp=return_period,
    )
    ...
    if validation_report and not validation_report.is_valid():
        gstn_json["_validation"] = validation_report.to_dict()

    json_bytes = json.dumps(gstn_json, indent=2).encode("utf-8")
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"GSTR1_{taxpayer_gstin or company_gstin or 'return'}_{return_period or timestamp}.json"
...
async def validate_gstr1_file(
    ...
    This endpoint performs structured validation checks using the GSTR1Engine validator:
    - Field-level validation (format, required fields, ranges)
    - Duplicate detection (invoice numbers, bill details)
    - Tax calculation verification
    - Cross-record validation
    - Statutory compliance checks (HSN/SAC, UQC, state codes)
    
    Returns structured validation report with:
    - Errors: Critical issues blocking GSTR-1 filing
    - Warnings: Non-critical issues for user review
    - Final status: Whether data is valid for filing
    """
    try:
        from india_compliance.gst_india.engine_core.engine import GSTR1Engine
        from india_compliance.gst_india.engine_core.input_adapter import ExcelInputAdapter
        ...
        # Extract validation reports
        input_validation_report = gstr1_data.get("input_validation_report", {})
        table_validation_report = gstr1_data.get("validation_report", {})
        
        # Determine overall status
        has_errors = (
            len(input_validation_report.get("errors", [])) > 0 or
            len(table_validation_report.get("errors", [])) > 0
        )
        ...
        validation_response = {
            ...
            "validation_report": {
                "input_validation": input_validation_report,
                "table_validation": table_validation_report,
                "final_status": "valid" if not has_errors else "invalid"
            },
            "summary": {
                "total_rows": len(df),
                "input_errors": len(input_validation_report.get("errors", [])),
                "input_warnings": len(input_validation_report.get("warnings", [])),
                "table_errors": len(table_validation_report.get("errors", [])),
                "table_warnings": len(table_validation_report.get("warnings", []))
            }
        }
        
        print("\nValidation Results:")
        print(f"  Input errors: {validation_response['summary']['input_errors']}")
        print(f"  Input warnings: {validation_response['summary']['input_warnings']}")
        print(f"  Table errors: {validation_response['summary']['table_errors']}")
        print(f"  Overall status: {validation_response['validation_report']['final_status']}")
        
        logger.info(f"GSTR-1 validation completed: status={validation_response['validation_report']['final_status']}, "
                   f"input_errors={validation_response['summary']['input_errors']}, "
                   f"warnings={validation_response['summary']['input_warnings']}")
        
        return validation_response
    
    except Exception as e:
        logger.exception(f"Error validating GSTR-1: {str(e)}")
        print(f"ERROR in /api/gstr1/validate: {str(e)}")
        import traceback
        traceback.print_exc()
        return {
            "success": False,
            ...
            "is_valid": False,
            "validation_report": {
                "input_validation": {"errors": [str(e)], "warnings": []},
                "table_validation": {"errors": [], "warnings": []},
                "final_status": "error"
            }
        }


@app.post("/api/gstr1/validate-db", tags=["GSTR1 Validation"])
async def validate_gstr1(
    ...
    DEPRECATED: Use /api/gstr1/validate (with file upload) instead for structured validation.
        ...
        return {
            ...
            "data": {
                ...
                "recent_filings": [
                    ...
                    for r in sorted(returns_db, key=lambda x: (x.get("filed_date") is None, x.get("filed_date", "")), reverse=True)[:5]
                ]
            }
        }
    except Exception as e:
...
async def list_reconciliations(
    ...
    # Sort by most recent
    results = sorted(results, key=lambda x: (x.get("created_at") is None, x.get("created_at", "")), reverse=True)
...
# ============ GSTR-3B CORE ENGINE ENDPOINTS ============

@app.get("/api/gstr3b/prepare", tags=["GSTR-3B"])
async def prepare_gstr3b_data(
    workspace_id: str,
    gstin: str,
    return_period: str,
    current_user: Optional[Dict[str, Any]] = Depends(get_current_user_optional)
    ...
    """
    Mock backend endpoint that ingests data and returns structured JSON
    to populate GSTR-3B summary engine.
    """
    try:
        # Realistic mock data that triggers Section 49 computation and validations
        gstr3b_data = {
            "outwardSupplies": {
                "taxableSupplies": { "taxableValue": 1250000.0, "igst": 180000.0, "cgst": 22500.0, "sgst": 22500.0, "cess": 0 },
                "zeroRatedSupplies": { "taxableValue": 350000.0, "igst": 0, "cgst": 0, "sgst": 0, "cess": 0 },
                "nilRatedSupplies": { "taxableValue": 25000.0, "igst": 0, "cgst": 0, "sgst": 0, "cess": 0 },
                "reverseChargeSupplies": { "taxableValue": 55000.0, "igst": 9900.0, "cgst": 0, "sgst": 0, "cess": 0 },
                "nonGstSupplies": { "taxableValue": 0 }
            },
            "interStateSupplies": {
                "unreg": [{"pos": "07-Delhi", "taxableValue": 50000, "igst": 9000}],
                "compDealer": [],
                "uin": []
            },
            "eligibleItc": {
                "itcAvailable": { "igst": 45000.0, "cgst": 12000.0, "sgst": 12000.0, "cess": 0 },
                "itcReversed": { "igst": 1500.0, "cgst": 0, "sgst": 0, "cess": 0 },
                "netItc": { "igst": 43500.0, "cgst": 12000.0, "sgst": 12000.0, "cess": 0 },
                "ineligibleItc": { "igst": 0, "cgst": 0, "sgst": 0, "cess": 0 }
            },
            "taxPayable": {
                "onOutwardSupplies": { "description": "Tax on outward supplies", "igst": 180000.0, "cgst": 22500.0, "sgst": 22500.0, "cess": 0 },
                "onReverseCharge": { "description": "Tax on reverse charge", "igst": 9900.0, "cgst": 0, "sgst": 0, "cess": 0 },
                "total": { "description": "Net tax payable", "igst": 189900.0, "cgst": 22500.0, "sgst": 22500.0, "cess": 0 }
            },
            "summary": {
                "totalInvoices": 18,
                "validatedInvoices": 18,
                "totalTaxableValue": 1680000.0,
                "totalTax": 234900.0
            }
        }
        ...
        return {
            ...
            "gstr1_data": {"b2b": []} # Prevent nil check issues on UI
        }
    except Exception as e:
        logger.error(f"Error preparing GSTR-3B: {str(e)}")
        raise HTTPException(status_code=500, detail="Failed to prepare data")


# In-memory storage for GSTR-3B states (matching GSTR-1 state behavior)
gstr3b_workflow_states: Dict[str, Dict[str, Any]] = {}

def get_gstr3b_state_key(workspace_id: str, gstin: str, return_period: str) -> str:
    return f"{workspace_id}::{gstin}::{return_period}"
...
@app.get("/api/gstr3b/state", tags=["GSTR-3B"])
async def get_gstr3b_state_route(
    workspace_id: str,
    gstin: str,
    return_period: str,
    current_user: Optional[Dict[str, Any]] = Depends(get_current_user_optional)
    ...
    try:
        state_key = get_gstr3b_state_key(workspace_id, gstin, return_period)
        if state_key in gstr3b_workflow_states:
            return {
                "success": True,
                "data": gstr3b_workflow_states[state_key]
            }
        raise HTTPException(status_code=404, detail="State not found")
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error fetching GSTR-3B state: {str(e)}")
        raise HTTPException(status_code=500, detail="Failed to fetch state")

@app.post("/api/gstr3b/state", tags=["GSTR-3B"])
async def save_gstr3b_state_route(
    request: GSTR3BWorkflowStateRequest,
    current_user: Optional[Dict[str, Any]] = Depends(get_current_user_optional)
):
    try:
        state_key = get_gstr3b_state_key(request.workspace_id, request.gstin, request.return_period)
        ...
        state_data = {
            ...
            "gstr3b_data": request.gstr3b_data,
            "gstr1_data": request.gstr1_data,
            "itc_data": request.itc_data,
            "tax_computation": request.tax_computation,
            "filing_result": request.filing_result,
            "status": request.status,
            "last_saved": request.last_saved,
            "updated_at": datetime.utcnow().isoformat() + "Z"
        }
        ...
        logger.error(f"Error saving GSTR-3B state: {str(e)}")
        raise HTTPException(status_code=500, detail="Failed to save state")

@app.delete("/api/gstr3b/state", tags=["GSTR-3B"])
async def delete_gstr3b_state_route(
    workspace_id: str,
    gstin: str,
    return_period: str,
    current_user: Optional[Dict[str, Any]] = Depends(get_current_user_optional)
    ...
    try:
        state_key = get_gstr3b_state_key(workspace_id, gstin, return_period)
        if state_key in gstr3b_workflow_states:
            del gstr3b_workflow_states[state_key]
            return {"success": True, "message": "GSTR-3B state cleared"}
        return {"success": True, "message": "State not found, nothing to delete"}
    except Exception as e:
        logger.error(f"Error deleting GSTR-3B state: {str(e)}")
        raise HTTPException(status_code=500, detail="Failed to delete state")

@app.get("/api/gstr3b/state/list", tags=["GSTR-3B"])
async def list_gstr3b_states_route(
    workspace_id: str,
    gstin: Optional[str] = None,
    limit: int = 10,
    current_user: Optional[Dict[str, Any]] = Depends(get_current_user_optional)
    ...
    try:
        states = []
        for key, state in gstr3b_workflow_states.items():
            if state.get("workspace_id") == workspace_id:
                if not gstin or state.get("gstin") == gstin:
                    states.append(state)
        ...
        logger.error(f"Error listing GSTR-3B states: {str(e)}")
...
class GSTR3BComputeRequest(BaseModel):
    gstin: str
    ret_period: str
    # Outward supplies (Table 3.1 aggregated)
    outward_supplies: Dict[str, Any] = {}   # {igst, cgst, sgst, cess}
    rcm_liability: Dict[str, Any] = {}      # 3.1(d) RCM
    # ITC (Table 4)
    itc_4a: Dict[str, Any] = {}             # 4(A) available
    itc_4b: Dict[str, Any] = {}             # 4(B) reversed
    # Options
    nil_return: bool = False
    gstr2b_import_id: Optional[str] = None  # optional GSTR-2B reference
    workspace_id: Optional[str] = None
    auto_save: bool = True


class GSTR3BSaveDraftRequest(BaseModel):
    gstin: str
    ret_period: str
    workspace_id: str
    ...
    override_flags: Optional[Dict[str, Any]] = None
    nil_return: bool = False
    status: str = "draft"   # draft | computed | confirmed


class GSTR3BConfirmSectionRequest(BaseModel):
    gstin: str
    ret_period: str
    workspace_id: str
    section: str        # "3_1_a", "4a", etc.
    values: Dict[str, Any]
...
@app.post("/api/gstr3b/validate", tags=["GSTR-3B"])
async def validate_gstr3b(
    request: GSTR3BValidateRequest,
    current_user: Optional[Dict[str, Any]] = Depends(get_current_user_optional),
):
    """
    Layer 1+3 Validation: Validate GSTR-3B data against all business rules.

    Returns error/warning/info categorized issues and can_proceed / can_file flags.
    Used by frontend to gate step progression and filing buttons.
    """
    try:
...
@app.post("/api/gstr3b/compute", tags=["GSTR-3B"])
async def compute_gstr3b(
    request: GSTR3BComputeRequest,
    current_user: Optional[Dict[str, Any]] = Depends(get_current_user_optional),
):
    """
    Layer 2 Business Logic: Compute GSTR-3B with real ITC utilization order.

    Implements Section 49 CGST Act utilization sequence:
      IGST ITC → IGST → CGST → SGST
      CGST ITC → CGST only
      SGST ITC → SGST only
      CESS ITC → CESS only

    Also computes 4(C) Net ITC, cash liability, carry-forward, and
    optionally fetches ITC from a GSTR-2B import if gstr2b_import_id is provided.
    Auto-saves draft to workspace if auto_save=True.
    """
    try:
        ...
        # If GSTR-2B import provided, try to pull ITC from it
        itc_4a = dict(request.itc_4a)
        if request.gstr2b_import_id and not any(itc_4a.values()):
            try:
                from india_compliance.gst_india.gstr3b_data import calculate_itc_available
                itc_result = calculate_itc_available({}, gstr2b_data=None)
                # Merge if we got data — otherwise keep user-provided values
                if any(v > 0 for v in itc_result.values()):
                    itc_4a.update({k: v for k, v in itc_result.items() if k in ("igst", "cgst", "sgst", "cess")})
            except Exception as itc_err:
                logger.warning(f"Could not fetch ITC from GSTR-2B import: {itc_err}")
        ...
        # Build full GSTR-3B payload for validation
        computed_data = {
            "gstin": request.gstin,
            "ret_period": request.ret_period,
            "nil_return": request.nil_return,
        ...
        # Auto-save draft if requested and workspace provided
        saved = False
...
async def confirm_gstr3b_section(
    ...
    Validates the section-level data before locking.
    """
    try:
        from india_compliance.gst_india.gstr3b_validation import validate_gstr3b as run_validation

        # Validate just this section's values
        partial_data = {
            "gstin": request.gstin,
            "ret_period": request.ret_period,
            request.section: request.values,
        }
        result = run_validation(partial_data)

        section_errors = [
            e.to_dict() for e in result.errors
            if e.section == request.section or e.section == "header"
        ]

        if section_errors:
            return {
                "success": False,
                "message": f"Section {request.section} cannot be confirmed — validation errors found.",
        ...
        return {
            "success": True,
            "message": f"Section {request.section} confirmed and locked.",
            "gstin": request.gstin,
            "ret_period": request.ret_period,
            "section": request.section,
            "confirmed": True,
            "confirmed_at": datetime.utcnow().isoformat() + "Z",
            "warnings": [w.to_dict() for w in result.warnings if w.section == request.section],
        }
    except Exception as e:
        logger.error(f"GSTR-3B confirm section error: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Confirm section failed: {str(e)}")


@app.get("/api/gstr3b/section-state", tags=["GSTR-3B"])
async def get_gstr3b_section_state(
    ...
    """
    Get GSTR-3B section enable/disable state plus nil_return flag.
    Used by the frontend to show/hide and enable/disable table sections.

    Returns:
        {nil_return, sections_enabled: {table_3, table_4, ...},
         auto_populated_fields, confirmed_sections}
    """
    try:
        # Build minimal data to derive section state
        data = {
            "gstin": gstin,
            "ret_period": ret_period,
            "nil_return": nil_return,
            # Defaults — if workspace data exists, fetch it
        }
        ...
        return {
            ...
            "auto_populated_fields": {
                "3_1_a": True,   # From GSTR-1
                "3_1_b": True,   # From GSTR-1
                "4a": True,      # From GSTR-2B
                "4b": False,     # Manual
            },
            "confirmed_sections": [],
        }
...
@app.post("/api/gstr3b/file", tags=["GSTR-3B"])
async def file_gstr3b(
    request: GSTR3BFileRequest,
    current_user: Optional[Dict[str, Any]] = Depends(get_current_user_optional),
):
    """
    Gate-validated GSTR-3B filing endpoint.

    Before filing:
      1. Runs full validation
      2. Checks all hard-error conditions (nil return non-zero, negative payable, ITC overuse)
      3. Only proceeds if can_file = True

    Returns filing status (stub — real GSTN API call would go here).
    """
    try:
        ...
        # Filing stub — in production this calls the GSTN API / GSP
        # e.g., gsp_client.file_gstr3b(gstin, ret_period, payload, otp)
        filing_ref = f"3B{request.gstin[:5]}{request.ret_period}{datetime.utcnow().strftime('%Y%m%d%H%M%S')}"

        logger.info(
            f"GSTR-3B filed: GSTIN={request.gstin}, Period={request.ret_period}, "
            f"Ref={filing_ref}"
        )
...
async def get_upload_history(
    ...
    """
    # Sort by most recent
    results = sorted(upload_history_db, key=lambda x: (x.get("created_at") is None, x.get("created_at", "")), reverse=True)

...
class BaseAPI:
    ...
    def __init__(self, *args, **kwargs):
        self.settings = document_cache.get_cached_doc("GST Settings")
    ...
    def _make_request(
        ...
        if method not in ("GET", "POST", "PUT"):
            throw(_("Invalid method {0}").format(method))
        ...
        try:
            ...
            try:
                response_json = response.json(object_hook=_dict)
            except Exception:
                pass
            ...
            if self.sandbox_mode and not flags.ic_sandbox_message_shown:
                msgprint(
                    _("GST API request was made in Sandbox Mode"),
                    alert=True,
                )
                flags.ic_sandbox_message_shown = True
    ...
    def handle_error_response(self, response_json):
        ...
        if not success_value and not self.is_ignored_error(response_json):
            throw(
                ...
                or as_json(response_json, indent=4),
    ...
    def is_ignored_error(self, response_json):
        # Override in subclass, return truthy value to stop throw
        pass
    ...
    def generate_request_id(self, length=12):
        return f"IC{generate_hash(length=length - 2)}".upper()
    ...
    def generate_app_key(self, service):
        app_key = generate_hash(length=32)
...
def check_scheduler_status():
    ...
    if flags.in_test or conf.developer_mode:
        return

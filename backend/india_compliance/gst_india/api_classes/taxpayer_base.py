...
# Use frappe replacements instead of frappe
...
class StaticResourcesAPI(BaseAPI):
    ...
    def get_gstn_public_certificate(self, error_message=None) -> str:
        ...
        if response.message == self.settings.gstn_public_certificate:
            throw(error_message or _("Public Certificate is already up to date"))
    ...
    def get_nic_public_key(self, error_message=None) -> str:
        ...
        if response.message == self.settings.nic_public_key:
            throw(error_message or _("Public Key is already up to date"))
...
class FilesAPI(BaseAPI):
    ...
    def process_response(self, response):
        ...
        data = json.loads(data, object_hook=_dict)
...
class TaxpayerAuthenticate(BaseAPI):
    ...
    def autheticate_with_otp(self, otp=None):
        if not otp:
            # in enqueue / cron job
            if getattr(local, "job", None):
                local.job.after_job.add(self.reset_auth_token)
            ...
            clear_document_cache("GST Settings")
    ...
    def reset_auth_token(self):
        ...
        if not flags.in_test:
            db.commit()  # nosemgrep - executed in after enqueue
    ...
    def get_public_ip(self):
        ...
        if not session_ip:
            throw(_("Could not fetch Public IP address."))
...
class TaxpayerBaseAPI(TaxpayerAuthenticate):
    ...
    def setup(self, company_gstin):
        if self.sandbox_mode:
            throw(_("Sandbox mode not supported for Returns API"))
        ...
        if response.get("data"):
            ...
            if response.get("hmac"):
                ...
                if hmac != response.hmac:
                    throw(_("HMAC mismatch"))

            response.result = parse_json(b64decode(decrypted_data).decode())
    ...
    def encrypt_request(self, json):
        ...
        if json.get("data"):
            b64_data = b64encode(as_json(json.get("data")).encode())
    ...
    def validate_auth_token(self):
        ...
        """
        if cache.get_value(f"authenticated_gstin:{self.company_gstin}"):
            return
    ...
    @staticmethod
    def get_fy():
        from india_compliance.gst_india.utils import getdate
        date = getdate()

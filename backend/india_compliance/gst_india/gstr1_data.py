...
def flt(value: Any, precision: int = 2) -> float:
    """Round a value to specified precision. Handles None and NaN properly."""
    if value is None:
        return 0.0
    try:
        val = float(value)
...
def normalize_row_fields(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Maps upstream field names to downstream expected field names, and tightly
    preserves correct numeric types for core tax calculation engine.
    """
    normalized = dict(row)

    # Scrub NaNs safely to prevent downstream string methods (like len) from crashing
    for k, v in list(normalized.items()):
        if pd.isna(v) or v is None or (isinstance(v, str) and v.lower() == "nan"):
            normalized[k] = ""

    # GSTIN normalization
    if "customer_gstin" in normalized and "gstin" not in normalized:
        normalized["gstin"] = normalized["customer_gstin"]

    # Invoice number normalization
    if "invoice_no" in normalized and "invoice_number" not in normalized:
        normalized["invoice_number"] = normalized["invoice_no"]
    elif "Invoice_No" in normalized and "invoice_number" not in normalized:
        normalized["invoice_number"] = normalized["Invoice_No"]

    # Rate normalization
    if "gst_rate" in normalized and "rate" not in normalized:
        normalized["rate"] = normalized["gst_rate"]
    elif "GST_Rate" in normalized and "rate" not in normalized:
        normalized["rate"] = normalized["GST_Rate"]

    # Quantity normalization
    if "qty" in normalized and "quantity" not in normalized:
        normalized["quantity"] = normalized["qty"]
    elif "Quantity" in normalized and "quantity" not in normalized:
        normalized["quantity"] = normalized["Quantity"]

    # HSN normalization
    if "hsn" in normalized and "hsn_code" not in normalized:
        normalized["hsn_code"] = normalized["hsn"]

    # Value normalization
    if "Invoice_Value" in normalized and "invoice_value" not in normalized:
        normalized["invoice_value"] = normalized["Invoice_Value"]
        
    if "Taxable_Value" in normalized and "taxable_value" not in normalized:
        normalized["taxable_value"] = normalized["Taxable_Value"]
        
    # Standard Metadata normalizations 
    if "Place_of_Supply" in normalized and "place_of_supply" not in normalized:
        normalized["place_of_supply"] = normalized["Place_of_Supply"]
        
    if "Reverse_Charge" in normalized and "reverse_charge" not in normalized:
        normalized["reverse_charge"] = normalized["Reverse_Charge"]
        
    if "Document_Type" in normalized and "document_type" not in normalized:
        normalized["document_type"] = normalized["Document_Type"]
        
    if "UOM" in normalized and "uom" not in normalized:
        normalized["uom"] = normalized["UOM"]

    # Tax amount robust alias mapping 
    tax_aliases = {
        'igst': ['igst_amount', 'IGST', 'integrated_tax', 'Integrated Tax', 'Integrated Tax Amount', 'joi'],
        'cgst': ['cgst_amount', 'CGST', 'central_tax', 'Central Tax', 'Central Tax Amount', 'joc'],
        'sgst': ['sgst_amount', 'SGST', 'state_tax', 'State/UT Tax', 'State Tax Amount', 'jos'],
        'cess': ['cess_amount', 'CESS', 'Cess', 'Cess Amount']
    }

    for std_key, aliases in tax_aliases.items():
        # Map aliases to standard key
        ...
        if std_key in normalized:
            ...
            if pd.isna(val) or val is None or str(val).strip() == '':
                ...
                try:
                    normalized[std_key] = float(str(val).replace(',', '').strip())
...
def generate_gstr1_tables(
    ...
    for original_row in clean_data:
        ...
        customer_gstin = str(row.get("gstin", "") or "").strip()
        ...
        if category == GSTR1_Category.EXP.value:
            shipping_bill_number = str(row.get("shipping_bill_number", "")).strip()
            shipping_bill_date = str(row.get("shipping_bill_date", "")).strip()
        ...
        uqc_code = str(row.get("uom", "") or row.get("uqc", "") or "").strip()
        if uqc_code:
        ...
        if rate_value == 0 and taxable_value > 0:
            gst_treatment = str(row.get("gst_treatment", "")).lower()
        ...
        # ADD 6: E-Commerce Operator Table
        operator_gstin = str(row.get("operator_gstin", "") or row.get("ecommerce_gstin", "") or "").strip()
        ...
        invoice_type = str(row.get("invoice_type", "")).lower()
    ...
    result = {
        ...
        "at": advances_received,  
        "txpd": advances_adjusted, 
        ...
        # --- OMNI-ALIAS SUMMARY FIX ---
        "summary": {
            # 1. Standard Python/Backend snake_case
            ...
            # 2. React/Frontend camelCase (Direct mapping for UI cards)
            "totalRecords": total_invoices,
            ...
            # 3. Shorthand camelCase (Alternative UI mapping)
            "taxableValue": round(total_taxable, 2),
            "igstAmount": round(total_igst, 2),
            "cgstAmount": round(total_cgst, 2),
            "sgstAmount": round(total_sgst, 2),
            "cessAmount": round(total_cess, 2),

            # 4. GSTN standard abbreviated schema keys
            "txnval": round(total_taxable, 2),

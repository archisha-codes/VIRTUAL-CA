...
# Static sales doctypes (previously from erpnext.stock.get_item_details)
ABBREVIATIONS = {
...
GST_CATEGORY_MAP = {
    ...
    "Registered Composition": "C",
    "Unregistered": "U",
    "SEZ": "E",
    "Overseas": "O",
    "UIN Holders": "F",
    "Deemed Export": "D",
    "Input Service Distributor": "I",
    "Tax Deductor": "T",
    "Tax Collector": "TC",
}
ACTION_MAP = {
...
E_INVOICE_MASTER_CODES_URL = "https://einvoice1.gst.gov.in/Others/MasterDownload.aspx"

# Additional Constants (simplified for lightweight usage)

ABBREVIATIONS_LIST = ["GIDC", "MIDC", "SEZ", "GST", "CGST", "SGST", "IGST", "HSN", "SAC"]

GST_ACCOUNT_FIELDS = ["igst_account", "cgst_account", "sgst_account", "cess_account"]

GST_INVOICE_NUMBER_FORMAT = r"^[A-Za-z0-9/-]+$"

PINCODE_FORMAT = r"^[1-9][0-9]{5}$"

SALES_DOCTYPES_LIST = ["Sales Invoice", "POS Invoice"]

TIMEZONE = "Asia/Kolkata"

# Simplified UOM mapping for basic usage
UOM_SIMPLE_MAP = {
    "NOS": "Number",
    "KGS": "Kilograms",
    "LTR": "Litres",
...
# State numbers (simplified version)
STATE_NUMBERS_SIMPLIFIED = {
    "Jammu and Kashmir": "01",
    "Himachal Pradesh": "02",
    "Punjab": "03",
    "Chandigarh": "04",
...
# Simplified pincode mapping
STATE_PINCODE_SIMPLE = {
    "Maharashtra": [(400, 449)],
    "Gujarat": [(360, 399)],
    "Karnataka": [(560, 599)],
    "Delhi": [(110, 110)],
...
# Simplified GSTIN formats
REGISTERED_SIMPLE = re.compile(r"^\d{2}[A-Z]{5}\d{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$")
UIN_HOLDERS = re.compile(r"^\d{2}[A-Z]{5}\d{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$")
TAX_DEDUCTOR = re.compile(r"^\d{2}[A-Z]{5}\d{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$")
TAX_COLLECTOR = re.compile(r"^\d{2}[A-Z]{5}\d{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$")
OVERSEAS_SIMPLE = re.compile(r"^O[A-Z0-9]{14}$")
...
# GST Tax Types
GST_TAX_TYPES = ("igst", "cgst", "sgst", "cess")

# Taxable Treatments
TAXABLE_GST_TREATMENTS = ("Taxable",)

GSTIN_FORMATS_SIMPLE = {
    "Registered Regular": REGISTERED_SIMPLE,
    "UIN Holders": UIN_HOLDERS,
    "Tax Deductor": TAX_DEDUCTOR,
    "Tax Collector": TAX_COLLECTOR,
    "Overseas": OVERSEAS_SIMPLE
}

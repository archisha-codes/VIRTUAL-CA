...
def classify_transaction(row: Dict[str, Any], company_gstin: str = "") -> str:
    ...
    gstin = get_scalar(row.get("gstin", "")) or ""
    pos = str(get_scalar(row.get("place_of_supply", ""))).lower()
    # Safe float conversion
    try:
        raw_val = get_scalar(row.get("invoice_value", 0))
        invoice_value = float(str(raw_val).strip().replace(',', ''))

...
def to_dict(data):
    """Convert data to a dict-like object (replacement for frappe._dict)"""
    if isinstance(data, dict):
        return type("DictLike", (), {**data, "get": lambda self, k, default=None: data.get(k, default)})()
    return data
...
class BooksDataMapper:
    ...
    def process_data_for_hsn_summary(self, grouped_data, prepared_data):
        ...
        for sub_category, hsn_key in grouped_data.items():
            ...
            # TODO: Replace with actual database lookup for HSN codes
            # This is a placeholder - frappe.get_all("GST HSN Code", ...) needs database schema
            descriptions = {}
            # Original code was:
            ...
            #     )
            # )
            # For now, using empty dict as placeholder - HSN descriptions will be empty
    ...
    def update_rounding_difference(self, prepared_data):
        ...
        # TODO: Replace with actual database lookup for currency_precision
        precision = None
    ...
    def get_already_filed_docs(self, m1_m2_subcategories):
        ...
        for log_name in log_names:
            ...
            if not gstr1_log.filed:
                ...
                # TODO: Replace with actual dict copy - frappe._dict(self.filters) needs Frappe
...
# =============================================================================
# Explicit Pydantic Models for Schema Validation (Govt JSON Format)
# =============================================================================
from pydantic import BaseModel, Field, conlist, confloat
from typing import List, Optional, Union

class GovItemDetails(BaseModel):
    txval: float
    rt: float
    iamt: Optional[float] = 0.0
    camt: Optional[float] = 0.0
    samt: Optional[float] = 0.0
    csamt: Optional[float] = 0.0

class GovItem(BaseModel):
    num: int
    itm_det: GovItemDetails

class GovB2BInvoice(BaseModel):
    inum: str
    idt: str
    val: float
    pos: str
...
class GovB2BEntity(BaseModel):
    ctin: str
    inv: List[GovB2BInvoice]

class GovB2CLInvoice(BaseModel):
    inum: str
    idt: str
    val: float
    pos: str
...
class GovB2CSInvoice(BaseModel):
    pos: str
    sply_ty: str
    txval: float
    rt: float
...
class GovEXPInvoice(BaseModel):
    inum: str
    idt: str
    val: float
    sbpcode: Optional[str] = None
    sbnum: Optional[str] = None
    sbdt: Optional[str] = None
    itms: List[GovItem]

class GovEXPEntity(BaseModel):
    exp_typ: str
    inv: List[GovEXPInvoice]

class GovCDNRNote(BaseModel):
    nt_num: str
    nt_dt: str
    nt_ty: str
    pos: str
    ...
    val: float
    itms: List[GovItem]
    p_gst: Optional[str] = "N"

class GovCDNREntity(BaseModel):
    ctin: str
    nt: List[GovCDNRNote]

class GovCDNURNote(BaseModel):
    typ: str
    nt_num: str
    nt_dt: str
    nt_ty: str
...
class GovDocType(BaseModel):
    doc_num: int
    doc_typ: str
    docs: List[GovDocDetail]

class GovDocs(BaseModel):
    doc_det: List[GovDocType]

class GSTR1GovSchema(BaseModel):
    gstin: str
    fp: str
    gt: float
    cur_gt: float
    b2b: Optional[List[GovB2BEntity]] = Field(default_factory=list)
    b2cl: Optional[List[GovB2CLInvoice]] = Field(default_factory=list)
    b2cs: Optional[List[GovB2CSInvoice]] = Field(default_factory=list)
    exp: Optional[List[GovEXPEntity]] = Field(default_factory=list)
    cdnr: Optional[List[GovCDNREntity]] = Field(default_factory=list)
    cdnur: Optional[List[GovCDNURNote]] = Field(default_factory=list)
    hsn: Optional[Dict[str, List[GovHSNEntry]]] = Field(default_factory=dict)
    doc_issue: Optional[GovDocs] = None

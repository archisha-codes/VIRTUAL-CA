...
def getdate(date):
    """Convert a date string or datetime to a date object robustly handling multiple formats."""
    if isinstance(date, datetime):
        return date.date()
    
    if isinstance(date, str):
        date_str = date.strip()
        # If it contains time (e.g., '2025-12-05 00:00:00'), split and take just the date part
        if ' ' in date_str:
            date_str = date_str.split(' ')[0]
        elif 'T' in date_str:
            date_str = date_str.split('T')[0]
            
        # List of expected date formats in real-world GST data
        formats = [
            "%Y-%m-%d",   # 2026-04-01
            "%d/%m/%Y",   # 01/04/2026
            "%d-%m-%Y",   # 01-04-2026
            "%m/%d/%Y",   # 04/01/2026 (US format sometimes found in ERP exports)
            "%Y/%m/%d"    # 2026/04/01
        ]
        ...
        raise ValueError(f"Time data '{date}' does not match any recognized format. Supported formats include YYYY-MM-DD and DD/MM/YYYY.")
...
def extract_state_code(gstin: str) -> str:
    """
    Extract the first two digits (state code) from a GSTIN.
    """
...
def get_b2c_limit(date):
    ...
    for limit_date, limit in B2C_LIMIT:
        limit_date_obj = getdate(limit_date)
        if date <= limit_date_obj:
            return limit

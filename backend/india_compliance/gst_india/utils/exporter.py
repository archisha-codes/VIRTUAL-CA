"""
Excel Exporter utilities for FastAPI.
All Frappe dependencies have been replaced with native Python equivalents.
"""
...
class ExcelExporter:
    ...
    def export(self, file_name):
        """
        Export workbook to binary file.
        TODO: In FastAPI, this should return a FileResponse or StreamingResponse.
        """
...
class Worksheet:
    ...
    def __init__(self):
        ...
        # Create filter_format and header_format
        self.filter_format = self.data_format.copy()
        self.filter_format["bg_color"] = None
        self.filter_format["bold"] = True
    ...
    def apply_format(self, row, column, **kwargs):
        ...
        if key == "is_total":
            style.update(
                {
                    "horizontal": self.data_format.get("horizontal", "general"),
                    "height": self.data_format.get("height", 20),
                }
            )
    ...
    def apply_style(self, row, column, style):
        ...
        cell.font = Font(
            name=style.get("font_family", "Calibri"),
            size=style.get("font_size", 9),
            bold=style.get("bold", False)
        )
        cell.alignment = Alignment(
            horizontal=style.get("horizontal", "general"),
            vertical=style.get("vertical", "center"),
        ...
        self.ws.row_dimensions[row].height = style.get("height", 20)
    ...
    def get_column_index(self, column_name):
        ...
        for idx, field in enumerate(self.headers, 1):
            if field.get("fieldname") == column_name:
                return idx

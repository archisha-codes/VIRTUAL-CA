...
def safe_float(value: Any, default: float = 0.0) -> float:
    """Safely convert value to float, handling strings, None, and NaN."""
    # Handle potential Series if duplicate columns exist
    if hasattr(value, "iloc"):
        value = value.iloc[0] if len(value) > 0 else default
    elif isinstance(value, (list, np.ndarray)) and not isinstance(value, str):
...
class ValidationReport:
    ...
    def add_result(self, result: ValidationResult):
        ...
        total_key = f"total_{result.severity.value}"
        self.summary[total_key] = self.summary.get(total_key, 0) + 1
    ...
    def has_warnings(self) -> bool:
        ...
        return self.summary.get("total_warning", 0) > 0
...
def validate_tax_calculation(
    ...
    **kwargs
    ...
    cess = safe_float(row.get("cess", 0))
...
def validate_amount_consistency(
    ...
    **kwargs
    ...
    invoice_val = safe_float(invoice_val)
    taxable = safe_float(taxable)
    total_tax = safe_float(igst) + safe_float(cgst) + safe_float(sgst) + safe_float(cess)
...
class ValidationEngine:
    ...
    def validate_dataframe(
        ...
        report.rules = [rule.to_dict() for rule in self.rules] + [
            {
                "name": "duplicate_invoice",
                "category": ValidationCategory.DUPLICATE.value,
        ...
        for row_index, (_, row) in enumerate(df.iterrows()):
            row_results = self.validate_row(row, row_index, supplier_gstin)

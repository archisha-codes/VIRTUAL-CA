...
export function AppHeader({ title = 'Dashboard' }: AppHeaderProps) {
  ...
  useEffect(() => {
    ...
    const matchingWorkspace = workspaces.find((workspace) => workspace.id === currentOrganization.id);
    if (matchingWorkspace && matchingWorkspace.id !== selectedWorkspace?.id) {
      setSelectedWorkspace(matchingWorkspace);
    }
  }, [currentOrganization, workspaces, selectedWorkspace?.id]);
  ...
  const fetchWorkspaces = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Handle both direct array or wrapped data object responses
        const workspaceData = data.data || data;
        setWorkspaces(workspaceData);
        if (workspaceData.length > 0) {
          const activeWorkspace = currentOrganization
            ? workspaceData.find((workspace: Workspace) => workspace.id === currentOrganization.id)
            : null;
          setSelectedWorkspace(activeWorkspace || workspaceData[0]);
        }
      }
    } catch (error) {
  ...
  const fetchClients = async () => {
    try {
      // The backend exposes clients directly at /clients, not /api/clients

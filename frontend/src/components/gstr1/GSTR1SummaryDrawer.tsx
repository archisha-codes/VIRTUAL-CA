...
const summariesItems = [
  ...
  { id: 'adv-adv', name: 'Tax Liabilities - Advance Received (11A(1), 11A(2))', type: 'item' },
  { id: 'adv-adj', name: 'Adjustment of Advances (11B(1), 11B(2))', type: 'item' },
  { id: 'hsn-b2b', name: 'HSN Summary of Outward Supplies (12)', type: 'item' },
  { id: 'doc', name: 'Document Series Summary', type: 'item' },
  { id: 'ecom-tcs', name: 'Liable to collect tax u/s 52 (TCS) (14(1))', type: 'item' },
  { id: 'ecom-pay', name: 'Liable to pay tax u/s 9(5) (14(2))', type: 'item' },
  { id: 'sup-unreg-reg', name: 'Supplies to unregistered recipients u/s 9(5) (15)', type: 'item' }
];
...
const amendmentsItems = [
  ...
  { id: 'a-ecom-tcs', name: 'Liable to collect tax u/s 52 (TCS) (14(1)) amendments', type: 'item' },
  { id: 'a-ecom-pay', name: 'Liable to pay tax u/s 9(5) (14(2)) amendments', type: 'item' },
  { id: 'a-sup-reg-unreg', name: 'Supplies to unregistered recipients u/s 9(5) (15) amendments', type: 'item' }
];
...
export default function GSTR1SummaryDrawer({
  ...
  const [docFilterSection, setDocFilterSection] = useState<string>('');
  const [docFilterDateFrom, setDocFilterDateFrom] = useState<string>('');
  const [docFilterDateTo, setDocFilterDateTo] = useState<string>('');
  ...
  const getRowData = (itemId: string) => {
    ...
    const dataMap: Record<string, any> = {
      ...
      'nil-exe': calculateSectionData(asItems(data['nil-exe'] || data.nil_exe || data.nil_exempted || (data.nil_exemp ? { taxable_value: data.nil_exemp.expt_amt, txval: data.nil_exemp.expt_amt } : null))),
      'nil-nil': calculateSectionData(asItems(data['nil-nil'] || data.nil_nil || data.nil_rated || (data.nil_exemp ? { taxable_value: data.nil_exemp.nil_amt, txval: data.nil_exemp.nil_amt } : null))),
      'nil-non': calculateSectionData(asItems(data['nil-non'] || data.nil_non || data.non_gst || (data.nil_exemp ? { taxable_value: data.nil_exemp.ngsup_amt, txval: data.nil_exemp.ngsup_amt } : null))),
      ...
      'hsn-b2b': calculateSectionData(asItems(data['hsn-b2b'] || data.hsn_b2b || data.hsn)),
      ...
      // Amendment sections - GSTN uses b2ba, b2cla, expa, cdnra, cdnura, b2csa suffixes
      'amend-b2b': calculateSectionData(asItems(
        data.b2ba?.flatMap((c: any) => c.invoices ? c.invoices : [c])
        || data['amend-b2b'] || data.amend_b2b || data.b2ba || []
      )),
      'amend-b2cla': calculateSectionData(asItems(
        data.b2cla || data['amend-b2cla'] || data.amend_b2cla || []
      )),
      'amend-exp': calculateSectionData(asItems(
        data.expa || data['amend-exp'] || data.amend_exp || []
      )),
      'amend-cdnr': calculateSectionData(asItems(
        data.cdnra?.flatMap((c: any) => c.notes ? c.notes : [c])
        || data['amend-cdnr'] || data.amend_cdnr || data.cdnra || []
      )),
      'amend-cdnur': calculateSectionData(asItems(
        data.cdnura || data['amend-cdnur'] || data.amend_cdnur || []
      )),
      'amend-sup-reg': calculateSectionData(asItems(
        data['amend-sup-reg'] || data.amend_sup_reg || []
      )),
      'amend-b2cs': calculateSectionData(asItems(
        data.b2csa || data['amend-b2cs'] || data.amend_b2cs || []
      )),
      'amend-adv': calculateSectionData(asItems(
        data.ata || data['amend-adv'] || data.amend_adv || []
      )),
      'amend-adj': calculateSectionData(asItems(
        data.txpda || data['amend-adj'] || data.amend_adj || []
      )),
      'a-ecom-tcs': calculateSectionData(asItems(
        data['a-ecom-tcs'] || data.a_ecom_tcs || []
      )),
      'a-ecom-pay': calculateSectionData(asItems(
        data['a-ecom-pay'] || data.a_ecom_pay || []
      )),
      'a-sup-reg-unreg': calculateSectionData(asItems(
        data['a-sup-reg-unreg'] || data.a_sup_reg_unreg || []
      )),
      'a-sup-unreg-unreg': calculateSectionData(asItems(
        data['a-sup-unreg-unreg'] || data.a_sup_unreg_unreg || []
      )),
    };
  ...
  const getGroupTotalData = (itemIds: string[]) => {
    let docCount = 0;
    let totalAmount = 0;
    let taxableAmount = 0;
    let totalTax = 0;
    ...
    itemIds.forEach(id => {
      ...
      const isAdjustment = id === 'adv-adj' || id === 'amend-adj';
      const factor = isAdjustment ? -1 : 1;
      
      docCount += Number(d.docCount || 0);
      totalAmount += factor * Number(d.totalAmount || 0);
      taxableAmount += factor * Number(d.taxableAmount || 0);
      totalTax += factor * Number(d.totalTax || 0);
      igst += factor * Number(d.igst || 0);
      cgst += factor * Number(d.cgst || 0);
      sgst += factor * Number(d.sgst || 0);
      cess += factor * Number(d.cess || 0);
    });
  ...
  const renderComparisonRow = (
    ...
    const bgColor = depth > 0 ? 'bg-blue-50/10' : 'bg-white';
    const isTotal = id === 'total';
    const isUploaded = summaryData?.filing_status === 'uploaded';

    const formatValue = (val: string | number, isCurrency = true) => {
      const num = Number(val || 0);
      if (num === 0) return '-';
      return isCurrency ? new Intl.NumberFormat('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(num) : num.toString();
    };

    // Action cell spans all 3 rows
    const actionCell = (
      <td className="py-2 px-4 text-center border-b border-slate-200 align-middle" rowSpan={3}>
        {!isGroup && !isTotal && (
          <button
            onClick={() => handleViewSection(id, name, toggleFn === toggleAmendmentGroup ? 'amendments' : (toggleFn === toggleSummaryGroup ? 'summaries' : 'grouped'))}
    ...
    return (
      <React.Fragment key={id}>
        {/* Your Data Row */}
        <tr className={`border-b border-slate-100 ${isTotal ? 'bg-slate-50' : bgColor}`}>
          <td className="py-0 px-3 border-r border-slate-200 align-middle" rowSpan={3} style={{ verticalAlign: 'middle', minWidth: 180 }}>
            <div className="flex items-center gap-1 py-2">
              {isGroup && toggleFn && (
                <button
                  onClick={() => toggleFn(id)}
                  className="mr-1 p-0.5 hover:bg-slate-200 rounded border border-slate-300 bg-white flex-shrink-0"
                >
              ...
              <span className={
                isTotal
                  ? "font-bold text-slate-900 text-[11px] uppercase"
                  : depth > 0
                  ? "text-slate-600 font-normal text-[11px] pl-3"
                  : "font-medium text-slate-700 text-[11px]"
              }>{name}</span>
            </div>
          </td>
          
          <td className="py-2 px-3 text-[10px] font-semibold text-slate-500 border-r border-slate-100 align-middle whitespace-nowrap bg-white">Your Data</td>
          <td className="py-2 px-3 text-[11px] text-center text-slate-600 border-r border-slate-100 align-middle">{formatValue(data.docCount, false)}</td>
          <td className="py-2 px-3 text-[11px] text-right text-slate-700 border-r border-slate-100 align-middle">{formatValue(data.totalAmount)}</td>
          <td className="py-2 px-3 text-[11px] text-right text-slate-700 border-r border-slate-100 align-middle">{formatValue(data.taxableAmount)}</td>
          <td className="py-2 px-3 text-[11px] text-right text-slate-700 border-r border-slate-100 align-middle">{formatValue(data.totalTax)}</td>
          <td className="py-2 px-3 text-[11px] text-right text-slate-700 border-r border-slate-100 align-middle">{formatValue(data.igst)}</td>
          <td className="py-2 px-3 text-[11px] text-right text-slate-700 border-r border-slate-100 align-middle">{formatValue(data.cgst)}</td>
          <td className="py-2 px-3 text-[11px] text-right text-slate-700 border-r border-slate-100 align-middle">{formatValue(data.sgst)}</td>
          <td className="py-2 px-3 text-[11px] text-right text-slate-700 border-r border-slate-200 align-middle">{formatValue(data.cess)}</td>
          
          {actionCell}
        </tr>

        {/* Uploaded to GSTN Row */}
        <tr className="border-b border-slate-100 bg-white/50">
          <td className="py-2 px-3 text-[10px] font-semibold text-slate-400 border-r border-slate-100 align-middle whitespace-nowrap">Uploaded to GSTN</td>
          <td className="py-2 px-3 text-[11px] text-center text-slate-400 border-r border-slate-100 align-middle">{isUploaded ? formatValue(data.docCount, false) : '-'}</td>
          <td className="py-2 px-3 text-[11px] text-right text-slate-400 border-r border-slate-100 align-middle">{isUploaded ? formatValue(data.totalAmount) : '-'}</td>
          <td className="py-2 px-3 text-[11px] text-right text-slate-400 border-r border-slate-100 align-middle">{isUploaded ? formatValue(data.taxableAmount) : '-'}</td>
          <td className="py-2 px-3 text-[11px] text-right text-slate-400 border-r border-slate-100 align-middle">{isUploaded ? formatValue(data.totalTax) : '-'}</td>
          <td className="py-2 px-3 text-[11px] text-right text-slate-400 border-r border-slate-100 align-middle">{isUploaded ? formatValue(data.igst) : '-'}</td>
          <td className="py-2 px-3 text-[11px] text-right text-slate-400 border-r border-slate-100 align-middle">{isUploaded ? formatValue(data.cgst) : '-'}</td>
          <td className="py-2 px-3 text-[11px] text-right text-slate-400 border-r border-slate-100 align-middle">{isUploaded ? formatValue(data.sgst) : '-'}</td>
          <td className="py-2 px-3 text-[11px] text-right text-slate-400 border-r border-slate-200 align-middle">{isUploaded ? formatValue(data.cess) : '-'}</td>
        </tr>

        {/* Difference Row */}
        <tr className="border-b border-slate-200 bg-blue-50/20">
          <td className="py-2 px-3 text-[10px] font-semibold text-blue-500 border-r border-slate-100 align-middle whitespace-nowrap">Difference</td>
          <td className="py-2 px-3 text-[11px] text-center text-blue-600 font-medium border-r border-slate-100 align-middle">{isUploaded ? '-' : formatValue(data.docCount, false)}</td>
          <td className="py-2 px-3 text-[11px] text-right text-blue-600 font-medium border-r border-slate-100 align-middle">{isUploaded ? '-' : formatValue(data.totalAmount)}</td>
          <td className="py-2 px-3 text-[11px] text-right text-blue-600 font-medium border-r border-slate-100 align-middle">{isUploaded ? '-' : formatValue(data.taxableAmount)}</td>
          <td className="py-2 px-3 text-[11px] text-right text-blue-600 font-medium border-r border-slate-100 align-middle">{isUploaded ? '-' : formatValue(data.totalTax)}</td>
          <td className="py-2 px-3 text-[11px] text-right text-blue-600 font-medium border-r border-slate-100 align-middle">{isUploaded ? '-' : formatValue(data.igst)}</td>
          <td className="py-2 px-3 text-[11px] text-right text-blue-600 font-medium border-r border-slate-100 align-middle">{isUploaded ? '-' : formatValue(data.cgst)}</td>
          <td className="py-2 px-3 text-[11px] text-right text-blue-600 font-medium border-r border-slate-100 align-middle">{isUploaded ? '-' : formatValue(data.sgst)}</td>
          <td className="py-2 px-3 text-[11px] text-right text-blue-600 font-medium border-r border-slate-200 align-middle">{isUploaded ? '-' : formatValue(data.cess)}</td>
        </tr>
      </React.Fragment>
    );
  };
  ...
  const renderRow = (
    ...
    return (
      <React.Fragment key={item.id}>
        {renderComparisonRow(item.name, item.id, data, isGroup, isExpanded, depth, toggleFn)}
  ...
  const renderMainSummary = () => {
    const groupedInvoicesIds = groupedInvoicesItems.map(i => i.id);
    const groupedInvoicesTotalData = getGroupTotalData(groupedInvoicesIds);

    const summariesIds = ['b2cs', 'nil-exe', 'nil-nil', 'nil-non', 'adv-adv', 'adv-adj'];
    const summariesTotalData = getGroupTotalData(summariesIds);

    const amendmentsIds = ['amend-b2b', 'amend-b2cla', 'amend-exp', 'amend-cdnr', 'amend-cdnur', 'amend-b2cs', 'amend-adv', 'amend-adj'];
    const amendmentsTotalData = getGroupTotalData(amendmentsIds);
      ...
      <div className="flex-1 overflow-y-auto bg-slate-100 p-4">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse border-b border-slate-200">
                <thead>
                  <tr className="bg-slate-50/90 border-b border-slate-200 text-slate-700 text-xs font-bold">
                    <th className="py-3 px-4 border-r border-slate-200 w-1/4" rowSpan={2}>Section</th>
                    <th className="py-3 px-4 text-center border-r border-slate-200 w-36" rowSpan={2}>Section Row</th>
                    <th colSpan={8} className="py-3 px-4 text-center border-r border-slate-200 text-slate-500 font-bold uppercase tracking-wider">
                      Your Data (Top Row) vs Uploaded to GSTN (Middle Row)
                    </th>
                    <th className="py-3 px-4 text-center w-24" rowSpan={2}>Action</th>
                  </tr>
                  <tr className="bg-slate-50/90 border-b border-slate-200 text-slate-600 text-[11px] font-bold">
                    <th className="py-2.5 px-4 border-r border-slate-100 text-center whitespace-nowrap"># Docs</th>
                    <th className="py-2.5 px-4 border-r border-slate-100 text-right whitespace-nowrap">Total Amount (₹)</th>
                    <th className="py-2.5 px-4 border-r border-slate-100 text-right whitespace-nowrap">Taxable Amount (₹)</th>
                    <th className="py-2.5 px-4 border-r border-slate-100 text-right whitespace-nowrap">Total Tax (₹)</th>
                    <th className="py-2.5 px-4 border-r border-slate-100 text-right whitespace-nowrap">IGST (₹)</th>
                    <th className="py-2.5 px-4 border-r border-slate-100 text-right whitespace-nowrap">CGST (₹)</th>
                    <th className="py-2.5 px-4 border-r border-slate-100 text-right whitespace-nowrap">SGST (₹)</th>
                    <th className="py-2.5 px-4 border-r border-slate-200 text-right whitespace-nowrap">CESS (₹)</th>
                  </tr>
                </thead>
                <tbody>
                  {renderComparisonRow('TOTAL', 'total', groupedInvoicesTotalData)}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse border-b border-slate-200">
                <thead>
                  ...
                  <tr className="bg-slate-50/90 border-b border-slate-200 text-slate-600 text-[11px] font-bold">
                    <th className="py-2.5 px-4 border-r border-slate-100 text-center whitespace-nowrap"># Rows</th>
                    <th className="py-2.5 px-4 border-r border-slate-100 text-right whitespace-nowrap">Total Amount (₹)</th>
                    <th className="py-2.5 px-4 border-r border-slate-100 text-right whitespace-nowrap">Taxable Amount (₹)</th>
                    <th className="py-2.5 px-4 border-r border-slate-100 text-right whitespace-nowrap">Total Tax (₹)</th>
                    <th className="py-2.5 px-4 border-r border-slate-100 text-right whitespace-nowrap">IGST (₹)</th>
                ...
                <tbody>
                  {renderComparisonRow('TOTAL', 'total', summariesTotalData)}
          ...
          <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden mb-8">
            <div className="px-4 py-3 border-b border-slate-200 bg-white">
              <h2 className="text-lg font-medium text-slate-800">Amendment Summary</h2>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse border-b border-slate-200">
                <thead>
                  ...
                  <tr className="bg-slate-50/90 border-b border-slate-200 text-slate-600 text-[11px] font-bold">
                    <th className="py-2.5 px-4 border-r border-slate-100 text-center whitespace-nowrap"># Docs / Rows</th>
                    <th className="py-2.5 px-4 border-r border-slate-100 text-right whitespace-nowrap">Total Amount (₹)</th>
                    <th className="py-2.5 px-4 border-r border-slate-100 text-right whitespace-nowrap">Taxable Amount (₹)</th>
                    <th className="py-2.5 px-4 border-r border-slate-100 text-right whitespace-nowrap">Total Tax (₹)</th>
                    <th className="py-2.5 px-4 border-r border-slate-100 text-right whitespace-nowrap">IGST (₹)</th>
                ...
                <tbody>
                  {renderComparisonRow('TOTAL', 'total', amendmentsTotalData)}
  ...
  const renderEditSummaryView = () => {
    ...
    const handleDeleteSelectedRows = () => {
      if (selectedRowIds.size === 0) {
        toast({ title: 'No rows selected', description: 'Select rows to delete by clicking on them.' });
        return;
      }
      setSummaryRows(prev => prev.filter(r => !selectedRowIds.has(r.id)));
      setSelectedRowIds(new Set());
      toast({ title: 'Rows Deleted', description: `${selectedRowIds.size} row(s) removed.` });
    };
    ...
    const handleAddRow = () => {
      ...
      setSummaryRows(prev => [...prev, newRow]);
      toast({ title: 'Row Added', description: 'A new row has been added to the table.' });
    };

    const handleExportSummaryCSV = () => {
      if (summaryRows.length === 0) {
        toast({ title: 'No data to export', description: 'Add rows before exporting.' });
        return;
      }
      const headers = ['Description', 'Nil Rated Supplies', 'Exempted', 'Non-GST Supplies'];
      const csvContent = [headers.join(','), ...summaryRows.map(r =>
        [r.type || '', r.val1 || '0.00', r.val2 || '0.00', r.val3 || '0.00'].map(v => `"${v}"`).join(',')
      )].join('\n');
      const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `${currentSectionView.replace(/\s+/g, '_')}_summary.csv`;
      link.click();
      URL.revokeObjectURL(link.href);
      toast({ title: 'CSV Exported', description: `Exported ${summaryRows.length} rows.` });
    };

    const handleSaveSummary = async () => {
      if (!workspaceId || !gstin || !returnPeriod) {
        toast({ title: 'Cannot save', description: 'Missing workspace, GSTIN, or return period.', variant: 'destructive' });
        return;
      }
      ...
      try {
        await saveGstr1State(workspaceId, gstin, returnPeriod, {
          currentStep: 'edited',
          stepData: { editedSection: currentSectionView },
          validationStatus: {},
          gstr1Tables: summaryData?.data || {},
          uploadResult: summaryData as unknown as Record<string, unknown>,
          classificationResult: null,
          validationResult: null,
          filingResult: null,
        });
        toast({ title: 'Summary Saved', description: 'Your changes have been saved successfully.' });
      } catch (err) {
        console.error('Failed to save summary:', err);
        toast({ title: 'Save Failed', description: 'Could not save the summary. Please try again.', variant: 'destructive' });
      } finally {
      ...
      <div className="flex-1 overflow-hidden flex flex-col bg-white">
        <div className="p-3 border-b flex justify-between items-center bg-white border-slate-200">
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-500 font-medium">{summaryRows.length} row(s)</span>
            {selectedRowIds.size > 0 && (
              <Badge variant="outline" className="text-[10px] bg-blue-50 text-blue-600 border-blue-200 font-medium">
          ...
          <div className="flex items-center gap-3">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="h-8 text-xs text-blue-600 border-blue-200 gap-1.5">
                  <FileText className="h-3 w-3" /> Actions <ChevronDown className="h-3 w-3" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-52">
                <DropdownMenuItem onClick={handleAddRow} className="text-xs gap-2 cursor-pointer">
                  <Plus className="h-3.5 w-3.5" /> Add New Row
                </DropdownMenuItem>
                <DropdownMenuItem onClick={handleDeleteSelectedRows} className="text-xs gap-2 cursor-pointer text-red-600 focus:text-red-600">
                  <Trash2 className="h-3.5 w-3.5" /> Delete Selected Rows
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => { setSelectedTemplate('virtualca'); setUploadDrawerOpen(true); }} className="text-xs gap-2 cursor-pointer">
            ...
            <Button
              className="bg-blue-600 hover:bg-blue-700 text-white h-8 text-xs px-5"
              onClick={handleSaveSummary}
              disabled={isSavingSummary}
            >
              {isSavingSummary ? <><Loader2 className="mr-1.5 h-3 w-3 animate-spin" /> Saving...</> : 'Save'}
            </Button>
          </div>
        </div>
        <div className="flex items-center justify-end px-4 py-2 bg-white">
          <div className="flex items-center gap-2">
            ...
            <div
              className="w-8 h-4 bg-blue-600 rounded-full flex items-center justify-end px-0.5 cursor-pointer"
          ...
          <table className="w-full text-left border-collapse border-b border-slate-200">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 text-[11px] font-semibold">
                <th className="py-3 px-4 w-10 text-center border-r border-slate-200 font-medium">
                  <Checkbox
                    className="border-slate-300"
                    checked={summaryRows.length > 0 && selectedRowIds.size === summaryRows.length}
                    onCheckedChange={(checked) => {
                      if (checked) {
                        setSelectedRowIds(new Set(summaryRows.map(r => r.id)));
                      } else {
                        setSelectedRowIds(new Set());
                      }
                    }}
            ...
            <tbody>
              {summaryRows.map((row) => (
                <tr key={row.id} className={`border-b border-slate-200 hover:bg-slate-50 group transition-colors ${selectedRowIds.has(row.id) ? 'bg-blue-50/50' : ''}`}>
                  ...
                  <td className="py-2.5 px-0 border-r border-slate-200 relative">
                    <input type="text" value={row.type || ''} onChange={e => updateRow(row.id, 'type', e.target.value)} className="w-full h-full absolute inset-0 bg-transparent outline-none focus:ring-1 focus:ring-blue-400 px-4 text-xs font-medium text-slate-700" placeholder="Enter description" />
                  </td>
        ...
        <div className="p-3 border-t border-slate-200 flex justify-between items-center bg-white shadow-sm mt-auto">
          <div className="flex items-center gap-2">
            <Button variant="outline" className="h-8 text-xs text-slate-600" onClick={handleAddRow}>
              <Plus className="mr-1 h-3 w-3" /> Add Row
            </Button>
          </div>
          <Button variant="outline" className="h-8 text-xs text-slate-600" onClick={() => setViewMode('summary')}>Back to Summary <ChevronRight className="ml-1 h-3 w-3" /></Button>
        </div>
      </div>
    );
  };
  ...
  const renderDocumentsView = () => {
    ...
    const filteredSectionData = sectionData.filter((row: any) => {
      if (docFilterSection) {
        const rowCtin = String(getRowValue(row, ['ctin', 'customer_gstin', 'gstin']) || '').toLowerCase();
        if (!rowCtin.includes(docFilterSection.toLowerCase())) return false;
      }
      if (docFilterDateFrom || docFilterDateTo) {
        const rowDate = String(getRowValue(row, ['idt', 'nt_dt', 'invoice_date', 'document_date']) || '');
        if (rowDate) {
          // Try to parse the date - handle common formats
          const dateStr = rowDate.replace(/(\d{2})-(\d{2})-(\d{4})/, '$3-$2-$1');
          const parsedDate = new Date(dateStr);
          if (!isNaN(parsedDate.getTime())) {
            if (docFilterDateFrom && parsedDate < new Date(docFilterDateFrom)) return false;
            if (docFilterDateTo && parsedDate > new Date(docFilterDateTo)) return false;
          }
        }
      }
      return true;
    ...
    return (
      <div className="flex-1 flex flex-col overflow-hidden bg-slate-50 font-sans">
        ...
        <div className="px-6 py-3 bg-slate-50/50 border-b border-slate-200 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            <Popover open={docFilterOpen} onOpenChange={setDocFilterOpen}>
              <PopoverTrigger asChild>
                <Button variant="outline" size="sm" className="h-8 text-xs gap-1.5 text-blue-600 border-blue-200 bg-white shadow-none">
                  <Filter className="h-3.5 w-3.5" /> Filters {(docFilterSection || docFilterDateFrom || docFilterDateTo) ? `(${[docFilterSection, docFilterDateFrom, docFilterDateTo].filter(Boolean).length} applied)` : ""}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-72 p-0" align="start">
                <div className="p-3 border-b border-slate-200 bg-slate-50">
                  <span className="text-xs font-bold text-slate-600 uppercase tracking-wider">Filter Documents</span>
                </div>
                <div className="p-3 space-y-3">
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-tight mb-1.5 block">Recipient GSTIN</label>
                    <Input
                      placeholder="Enter GSTIN"
                      value={docFilterSection}
                      onChange={(e) => setDocFilterSection(e.target.value)}
                      className="h-8 text-xs"
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-tight mb-1.5 block">Date From</label>
                      <Input
                        type="date"
                        value={docFilterDateFrom}
                        onChange={(e) => setDocFilterDateFrom(e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div>
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-tight mb-1.5 block">Date To</label>
                      <Input
                        type="date"
                        value={docFilterDateTo}
                        onChange={(e) => setDocFilterDateTo(e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                  </div>
                  <div className="flex gap-2 pt-1">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        setDocFilterOpen(false);
                        toast({ title: 'Filters Applied', description: 'Document filters have been applied.' });
                      }}
                    >
                      Apply
                    </Button>
            ...
            {(docFilterSection || docFilterDateFrom || docFilterDateTo) && (
              <div className="flex gap-1">
                {docFilterSection && (
                  <Badge variant="outline" className="h-7 bg-white text-slate-500 border-slate-200 font-medium px-2 py-0 shadow-none">
                    GSTIN: {docFilterSection.substring(0, 8)}... <X className="ml-1 h-3 w-3 cursor-pointer" onClick={() => setDocFilterSection('')} />
                  </Badge>
                )}
                {(docFilterDateFrom || docFilterDateTo) && (
                  <Badge variant="outline" className="h-7 bg-white text-slate-500 border-slate-200 font-medium px-2 py-0 shadow-none">
                    Date filter <X className="ml-1 h-3 w-3 cursor-pointer" onClick={() => { setDocFilterDateFrom(''); setDocFilterDateTo(''); }} />
                  </Badge>
                )}
              </div>
            )}
            {selectedDocIndices.size > 0 && (
              <Badge variant="outline" className="h-7 bg-blue-50 text-blue-600 border-blue-200 font-medium px-2 py-0 shadow-none">
                {selectedDocIndices.size} selected
              </Badge>
            )}
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs text-slate-500 font-medium whitespace-nowrap">1 - {filteredSectionData.length} of {sectionData.length}</span>
            ...
            <Button
              variant="outline"
              size="sm"
              className="h-8 text-xs text-blue-600 border-blue-200 bg-blue-50/10 font-bold border-dashed shadow-none"
              onClick={() => {
                if (filteredSectionData.length === 0) {
                  toast({ title: 'No data to download', description: 'There are no documents in this section.' });
                  return;
                }
                const headers = ['Document Number', 'Document Type', 'Section', 'Document Date', 'Recipient GSTIN', 'Recipient Name', 'Place of Supply', 'Taxable Value', 'IGST', 'CGST', 'SGST', 'CESS', 'Total Value'];
                const csvRows = filteredSectionData.map(row => {
                  const lineItems = row.itms || row.items || row.line_items || [];
                  const sourceItems = lineItems.length > 0 ? lineItems : [row];
                  const txval = sourceItems.reduce((s: number, it: any) => s + normalizeNumericValue(getRowValue(it, ['txval', 'taxable_value'])), 0);
                  const igst = sourceItems.reduce((s: number, it: any) => s + normalizeNumericValue(getRowValue(it, ['iamt', 'igst'])), 0);
                  const cgst = sourceItems.reduce((s: number, it: any) => s + normalizeNumericValue(getRowValue(it, ['camt', 'cgst'])), 0);
                  const sgst = sourceItems.reduce((s: number, it: any) => s + normalizeNumericValue(getRowValue(it, ['samt', 'sgst'])), 0);
                  const cess = sourceItems.reduce((s: number, it: any) => s + normalizeNumericValue(getRowValue(it, ['csamt', 'cess'])), 0);
                  const total = normalizeNumericValue(getRowValue(row, ['val', 'invoice_value', 'total_amount'])) || (txval + igst + cgst + sgst + cess);
                  return [
                    getRowValue(row, ['inum', 'nt_num', 'invoice_number']) || '',
                    getRowValue(row, ['inv_typ', 'nt_typ', 'document_type']) || '',
                    currentSectionId.toUpperCase(),
                    getRowValue(row, ['idt', 'nt_dt', 'invoice_date']) || '',
                    getRowValue(row, ['ctin', 'customer_gstin', 'gstin']) || '',
                    getRowValue(row, ['customer_name', 'name']) || '',
                    getRowValue(row, ['pos', 'place_of_supply']) || '',
                    txval.toFixed(2), igst.toFixed(2), cgst.toFixed(2), sgst.toFixed(2), cess.toFixed(2), total.toFixed(2),
                  ].map(v => `"${v}"`).join(',');
                });
                ...
                const link = document.createElement('a');
                link.href = URL.createObjectURL(blob);
                link.download = `${currentSectionId}_doc_report.csv`;
                link.click();
                URL.revokeObjectURL(link.href);
                toast({ title: 'Report Downloaded', description: `Exported ${filteredSectionData.length} documents as CSV.` });
              }}
            >
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-52">
                <DropdownMenuItem
                  ...
                  onClick={() => {
                    const newDoc = {
                      inum: `NEW-${Date.now().toString().slice(-6)}`,
                    ...
                    setIsEditMode(true);
                    setViewMode('document-details');
                    toast({ title: 'New Document', description: 'Fill in the details for the new document.' });
                  }}
                >
                  <Plus className="h-3.5 w-3.5" /> Add New Document
                </DropdownMenuItem>
                <DropdownMenuItem
                  className="text-xs gap-2 cursor-pointer text-red-600 focus:text-red-600"
                  onClick={() => {
                    if (selectedDocIndices.size === 0) {
                      toast({ title: 'No documents selected', description: 'Select documents using the checkboxes first.' });
                      return;
                    }
                    toast({ title: 'Documents Deleted', description: `${selectedDocIndices.size} document(s) marked for deletion.` });
                ...
                <DropdownMenuItem
                  ...
                  onClick={() => {
                    loadSummaryData();
                    toast({ title: 'Refreshing', description: 'Reloading data from the server...' });
                  }}
                >
                ...
                <DropdownMenuItem
                  ...
                  onClick={() => {
                    if (sectionData.length === 0) {
                      toast({ title: 'No data', description: 'No documents to export.' });
                      return;
                    }
                    const headers = ['Document Number', 'Document Type', 'Document Date', 'GSTIN', 'Name', 'Place of Supply'];
                    const csvRows = sectionData.map(row => [
                      getRowValue(row, ['inum', 'nt_num']) || '',
                      getRowValue(row, ['inv_typ', 'nt_typ']) || '',
                      getRowValue(row, ['idt', 'nt_dt']) || '',
                      getRowValue(row, ['ctin', 'customer_gstin']) || '',
                      getRowValue(row, ['customer_name', 'name']) || '',
                      getRowValue(row, ['pos', 'place_of_supply']) || '',
                    ].map(v => `"${v}"`).join(','));
                    const csvContent = [headers.join(','), ...csvRows].join('\n');
                    ...
                    const link = document.createElement('a');
                    link.href = URL.createObjectURL(blob);
                    link.download = `${currentSectionId}_all_documents.csv`;
                    link.click();
                    URL.revokeObjectURL(link.href);
                    toast({ title: 'Exported', description: `Exported ${sectionData.length} documents.` });
                  }}
                >
                  <FileSpreadsheet className="h-3.5 w-3.5" /> Export All as CSV
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
          ...
          <table className="w-full text-left border-collapse min-w-[1200px]">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 text-[11px] font-bold uppercase tracking-wider sticky top-0 bg-slate-50 z-10">
                <th className="py-3 px-4 w-12 text-center border-r border-slate-200">
                  <Checkbox
                    className="border-slate-300"
                    checked={filteredSectionData.length > 0 && selectedDocIndices.size === filteredSectionData.length}
                    onCheckedChange={(checked) => {
                      if (checked) {
                        setSelectedDocIndices(new Set(filteredSectionData.map((_: any, i: number) => i)));
                      } else {
                        setSelectedDocIndices(new Set());
                      }
                    }}
            ...
            <tbody>
              {filteredSectionData.length > 0 ? filteredSectionData.map((row: any, idx: number) => {
                ...
                return (
                  <tr key={idx} className={`border-b border-slate-100 hover:bg-slate-50/80 transition-colors text-xs text-slate-600 ${selectedDocIndices.has(idx) ? 'bg-blue-50/50' : ''}`}>
                    <td className="py-3 px-4 text-center border-r border-slate-100/50">
                      <Checkbox
                        className="border-slate-300"
                        checked={selectedDocIndices.has(idx)}
                        onCheckedChange={(checked) => {

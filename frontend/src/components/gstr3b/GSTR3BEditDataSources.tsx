...
const GSTR3BEditDataSources: React.FC<GSTR3BEditDataSourcesProps> = ({ 
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[100vw] w-[100vw] h-[100vh] flex flex-col p-0 overflow-hidden border-none shadow-none rounded-none bg-white focus:outline-none" onPointerDownOutside={(e) => e.preventDefault()} onEscapeKeyDown={(e) => e.preventDefault()}>
        
        {/* Top Header Row */}
        <div className="flex-none bg-[#F5F5F5] border-b border-slate-200 px-6 h-[48px] flex items-center justify-between z-[110]">
          <div className="text-[13px] font-bold text-[#333333] uppercase tracking-wide">
        ...
        {/* Content Section */}
        <div className="flex-1 flex flex-col overflow-hidden px-12 py-8 bg-white overflow-y-hidden">
          ...
          <div className="flex justify-between items-start mb-8 shrink-0">
            <div className="space-y-1.5">
              ...
              <p className="text-[14px] text-slate-500 font-medium">
                Government recommends <span className="font-bold text-slate-800 underline decoration-slate-300">G1</span> and <span className="font-bold text-slate-800 underline decoration-slate-300">2B</span> data to fill this table.
              </p>
            </div>
          ...
          <div className="flex-1 overflow-hidden border border-[#D9D9D9] rounded-sm bg-white shadow-sm flex flex-col">
            <div className="overflow-x-auto overflow-y-auto w-full flex-1 custom-scrollbar-thin">
              <Table className="border-collapse min-w-[5000px] table-fixed w-full">
                <TableHeader className="bg-[#F9FAFB] sticky top-0 z-50 border-b border-[#D9D9D9]">
                  <TableRow className="hover:bg-transparent border-none">
                    {/* Pillar 1: Checkbox & Business Header */}
                    <TableHead className="w-[50px] bg-[#F9FAFB] sticky left-0 z-[60] border-r border-[#D9D9D9] h-[120px] shadow-[2px_0_4px_rgba(0,0,0,0.04)]">
                      <div className="flex justify-center pt-8">
                        <Checkbox className="rounded-none border-[#D9D9D9] h-4 w-4" />
                      </div>
                    </TableHead>

                    <TableHead className="w-[420px] border-r border-[#D9D9D9] font-black text-[#555555] uppercase tracking-[0.2em] text-[10px] text-center sticky left-[50px] bg-[#F9FAFB] z-[60] h-[120px] shadow-[2px_0_4px_rgba(0,0,0,0.04)]">
                       <span className="block pt-8">BUSINESS</span>
                    </TableHead>

                    {/* Header: TABLE 3.1 A,B,C & E */}
                    <TableHead className="w-[340px] border-r border-[#D9D9D9] font-black text-[#555555] uppercase tracking-[0.1em] text-[10px] text-center px-6 pt-8">
                      TABLE 3.1 A,B,C & E
                    </TableHead>

                    {/* Group Header: TABLE 3.1 and 3.2 */}
                    <TableHead className="w-[720px] border-r border-[#D9D9D9] p-0" colSpan={2}>
                      <div className="flex flex-col h-full uppercase tracking-[0.1em] text-[10px] font-black">
                        <div className="border-b border-[#D9D9D9] px-4 py-3 text-[#555555] text-center bg-[#F3F4F6] mt-1">
                          TABLE 3.1 and 3.2
                        </div>
                        <div className="flex flex-1">
                          <div className="flex-1 border-r border-[#D9D9D9] px-4 py-6 text-[#666666] text-center flex items-center justify-center leading-tight">
                            TABLE 3.1 D INWARD SUPPLIES - RCM
                          </div>
                          <div className="flex-1 px-4 py-6 text-[#666666] text-center flex items-center justify-center">
                            TABLE 3.2 OUTWARD SUPPLIES
                          </div>
                        </div>
                      </div>
                    ...
                    <TableHead className="w-[2560px] border-r border-[#D9D9D9] p-0" colSpan={8}>
                        ...
                        <div className="flex flex-1">
                          {[
                            ...
                            <div 
                              key={idx} 
                              className={cn(
                                "w-[320px] px-8 py-4 text-[#666666] text-center flex items-center justify-center leading-snug",
                                idx < 7 ? "border-r border-[#D9D9D9]" : ""
                              )}
                            >
                              {label}
                            </div>
                    ...
                    <TableHead className="w-[360px] border-r border-[#D9D9D9] font-black text-[#555555] uppercase tracking-[0.05em] text-[9px] text-center px-6 pt-8 leading-tight">
                      TABLE 5 INWARD SUPPLIES (EXEMPT, NIL-RATED, NON-GST)
                    </TableHead>

                    {/* Header: TABLE 5.1.1 INTEREST */}
                    <TableHead className="w-[280px] font-black text-[#555555] uppercase tracking-[0.1em] text-[10px] text-center px-6 pt-8">
                      TABLE 5.1.1 INTEREST
                    </TableHead>
                  </TableRow>
                </TableHeader>
                ...
                <TableBody>
                  ...
                  <TableRow className="bg-[#F0F7FF] hover:bg-[#EAF3FF] border-b border-[#CADEFF] h-[72px]">
                    <TableCell className="border-r border-[#CADEFF] sticky left-0 z-40 bg-inherit shadow-[2px_0_4px_rgba(0,0,0,0.02)]">
                      <div className="flex justify-center">
                        <Checkbox className="rounded-none border-blue-300" />
                      </div>
                    </TableCell>
                    <TableCell className="border-r border-[#CADEFF] sticky left-[50px] z-40 bg-inherit shadow-[2px_0_4px_rgba(0,0,0,0.02)] py-4">
                      <div className="flex items-center gap-4 pl-4 pr-6">
                         <button 
                           onClick={() => setIsExpanded(!isExpanded)}
                           className="flex items-center justify-center w-5 h-5 rounded-sm border-2 border-[#1a1a1a] bg-white text-blue-600 shadow-sm transition-transform active:scale-95"
                         >
                           {isExpanded ? <Minus className="h-4 w-4" strokeWidth={5} /> : <Plus className="h-4 w-4" strokeWidth={5} />}
                         </button>
                         <span className="text-[14px] font-black text-[#1a2b4b] uppercase tracking-tight">{businessName}</span>
                      </div>
                    </TableCell>
                  ...
                  {/* GSTIN Row (Child) */}
                  {isExpanded && (
                    <TableRow className="bg-white hover:bg-slate-50 border-b border-[#D9D9D9] h-[72px]">
                      <TableCell className="border-r border-slate-100 sticky left-0 z-40 bg-inherit shadow-[2px_0_4px_rgba(0,0,0,0.02)]">
                        <div className="flex justify-center">
                          <Checkbox className="rounded-none border-slate-200" />
                        </div>
                      </TableCell>
                      <TableCell className="border-r border-slate-100 pl-20 sticky left-[50px] z-40 bg-inherit shadow-[2px_0_4px_rgba(0,0,0,0.02)] py-4">
                        <span className="text-[14px] font-bold text-slate-500">{gstin}</span>
                      </TableCell>
        ...
        {/* Footer */}
        <div className="flex-none px-12 py-6 border-t border-slate-200 bg-white flex items-center justify-end gap-6 z-[110]">
          <Button 
            variant="ghost" 
            onClick={() => {
              // Discard changes logic here (optional reset selections)
              console.log("Discarding changes...");
            }} 
            className="text-[#666666] font-black hover:bg-slate-50 px-8 h-12 tracking-[0.1em] text-[12px] uppercase transition-colors"
          >
            DISCARD CHANGES
          </Button>
          <Button 
            className="bg-[#007AFF] hover:bg-[#0062CC] text-white font-black px-16 h-[48px] shadow-lg shadow-blue-100 transition-all active:scale-[0.98] uppercase tracking-[0.1em] text-[13px] rounded-sm" 
            onClick={() => {

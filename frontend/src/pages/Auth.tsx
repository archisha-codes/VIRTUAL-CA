...
export default function Auth() {
  ...
  const handleOAuthLogin = async (provider: 'google' | 'github') => {
    ...
    try {
      ...
      console.log('OAuth login to:', provider, 'with redirect:', `${origin}/dashboard`);
      ...
      if (error) {
        ...
        toast({
          title: 'Login failed',
          description: error.message + '. If using Google OAuth, please configure it in Supabase Dashboard (Authentication → Providers → Google)',
          variant: 'destructive',
        });
      }
    } catch (err) {
      ...
      toast({
        title: 'Login failed',
        description: 'Unable to login with ' + provider + '. Please check Supabase OAuth configuration.',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(null);
              ...
              <TabsContent value="login">
                ...
                <Form {...loginForm}>
                  <form onSubmit={loginForm.handleSubmit(onLogin)} className="space-y-4">
                    ...
                    <Button type="submit" className="w-full bg-gradient-to-r from-olive-600 to-burgundy-700 hover:from-olive-700 hover:to-burgundy-800" disabled={!!isLoading}>

...
/**
 * Validate GSTR-1 data against GST rules
...
export async function getWorkspaces(userId: string): Promise<Workspace[]> {
  const response = await fetch(`${API_BASE_URL}/workspaces?user_id=${encodeURIComponent(userId)}`, {
    method: 'GET',
    headers: await getAuthHeaders(),
  });
...
export async function getWorkspaceGstins(workspaceId: string): Promise<WorkspaceGSTIN[]> {
  const response = await fetch(`${API_BASE_URL}/workspaces/${workspaceId}/gstins`, {
    method: 'GET',
    headers: await getAuthHeaders(),
  });
...
export async function createWorkspaceGstin(
  ...
  const response = await fetch(`${API_BASE_URL}/workspaces/${workspaceId}/gstins?user_id=${encodeURIComponent(userId)}`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      ...await getAuthHeaders(),
...
export async function deleteWorkspaceGstin(
  ...
  const response = await fetch(`${API_BASE_URL}/gstins/${gstinId}?user_id=${encodeURIComponent(userId)}&workspace_id=${encodeURIComponent(workspaceId)}`, {
    method: 'DELETE',
    headers: await getAuthHeaders(),
  });
  return handleResponse<{ message: string }>(response);
...
export async function addWorkspaceMember(
  ...
  const response = await fetch(`${API_BASE_URL}/workspaces/${workspaceId}/members?user_id=${encodeURIComponent(userId)}`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      ...await getAuthHeaders(),
...
export async function removeWorkspaceMember(
  ...
  const response = await fetch(`${API_BASE_URL}/workspaces/${workspaceId}/members/${memberUserId}?user_id=${encodeURIComponent(userId)}`, {
    method: 'DELETE',
    headers: await getAuthHeaders(),
  });
  return handleResponse<{ message: string }>(response);
...
export async function switchWorkspaceGstin(
  ...
  const response = await fetch(`${API_BASE_URL}/workspaces/${workspaceId}/switch-gstin?user_id=${encodeURIComponent(userId)}&gstin_id=${encodeURIComponent(gstinId)}`, {
    method: 'POST',
    headers: await getAuthHeaders(),
  });
  return handleResponse<{ message: string }>(response);
...
export async function getBusinessesWithGstins(
  ...
  // Get workspace details with GSTINs
  const workspace = await getWorkspace(workspaceId);
  
  // Get connection status for all GSTINs in workspace
  const connectionStatus = await getGSTINConnectionStatus(workspaceId);
  
  // Create a map of GSTIN to connection status
  const statusMap = new Map<string, boolean>();
  if (connectionStatus.success && connectionStatus.data) {
    connectionStatus.data.forEach(status => {
      statusMap.set(status.gstin, status.is_connected);
    });
  }
  
  // Group GSTINs by legal_name (company)
  const businessMap = new Map<string, BusinessWithGstins>();
  
  workspace.gstins.forEach(gstin => {
    const businessName = gstin.legal_name || gstin.trade_name || 'Unknown Business';
    // Extract PAN from GSTIN (first 10 characters without special chars)
    const pan = gstin.gstin.substring(0, 10).toUpperCase();
    ...
    business.gstins.push({
      ...
      gstin: gstin.gstin,
      state: gstin.state,
      status: gstin.status === 'active' ? 'Regular' : 
               gstin.registration_type === 'Composite' ? 'Composite' : 'Regular',
      isConnected: statusMap.get(gstin.gstin) || false
    });
  });
  
  return {
    success: true,
    data: Array.from(businessMap.values())
  };
}
